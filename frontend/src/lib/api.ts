...
/**
 * Review API endpoints (frontend stubs - backend not yet implemented)
 * These will integrate with the backend review service once available.
 */
export const reviewApi = {
  /**
   * Start a review for a task attempt
   */
  startReview: async (_attemptId: string, _options?: {
    profileId?: string;
    promptTemplate?: string;
  }): Promise<{ processId: string }> => {
    // TODO: Implement when backend is ready
    // const response = await makeRequest(`/api/task-attempts/${attemptId}/review`, {
    //   method: 'POST',
    //   body: JSON.stringify(options),
    // });
    // return handleApiResponse<{ processId: string }>(response);
    throw new Error('Review API not yet implemented');
  },

  /**
   * Retry a failed review
   */
  retryReview: async (_attemptId: string): Promise<{ processId: string }> => {
    // TODO: Implement when backend is ready
    throw new Error('Review API not yet implemented');
  },

  /**
   * Skip/cancel a pending or running review
   */
  skipReview: async (_attemptId: string): Promise<void> => {
    // TODO: Implement when backend is ready
    throw new Error('Review API not yet implemented');
  },

  /**
   * Get review status for a task attempt
   */
  getStatus: async (_attemptId: string): Promise<{
    status: 'none' | 'pending' | 'running' | 'completed' | 'failed' | 'skipped';
  ...
  /**
   * Get review feedback for a completed review
   */
  getFeedback: async (_attemptId: string): Promise<{
    items: Array<{
      id: string;
